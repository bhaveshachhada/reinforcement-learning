...
class FrozenLakeTDAgent:
    def __init__(
        ...
        self.q: np.ndarray = np.zeros(
            (self.env.observation_space.n, self.env.action_space.n), dtype=np.float64
        )
...
def setup_logger() -> logging.Logger:
    ...
    file_handler = logging.FileHandler("logs/frozenlake.log", mode="w")
...
def main():
    ...
    environment = gym.wrappers.RecordEpisodeStatistics(
        environment, buffer_length=NUM_EPISODES
    )
    ...
    for episode in track(range(NUM_EPISODES), description="Training..."):
        ...
        agent.decay_epsilon()
        logger.info(
            f"Episode: {episode}, Eps: {agent.epsilon}, trace: {' '.join(str(s) for s in trace)}"
        )
    ...
    reward_moving_average = get_moving_avgs(
        environment.return_queue, rolling_length, "valid"
    )
    axs[0].plot(range(len(reward_moving_average)), reward_moving_average)

    axs[1].set_title("Episode lengths")
    length_moving_average = get_moving_avgs(
        environment.length_queue, rolling_length, "valid"
    )
    axs[1].plot(range(len(length_moving_average)), length_moving_average)

    axs[2].set_title("Training Error")
    training_error_moving_average = get_moving_avgs(
        agent.training_errors, rolling_length, "same"
    )
    axs[2].plot(
